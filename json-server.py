    ...
    def do_POST(self):
        ...
        if url["requested_resource"] == "ships":
            ...
            if successfully_created:
                return self.response(
                    "Your ship has been created.",
                    status.HTTP_201_SUCCESS_CREATED.value,
                )
            else:
                return self.response(
                    "Unable to create ship.", status.HTTP_500_SERVER_ERROR.value
                )
        elif url["requested_resource"] == "docks":
            successfully_created = create_dock(request_body)
            if successfully_created:
                return self.response(
                    "Your dock has been created.",
                    status.HTTP_201_SUCCESS_CREATED.value,
                )
            else:
                return self.response(
                    "Unable to create dock.", status.HTTP_500_SERVER_ERROR.value
                )
        elif url["requested_resource"] == "haulers":
            successfully_created = create_hauler(request_body)
            if successfully_created:
                return self.response(
                    "Your hauler has been created.",
                    status.HTTP_201_SUCCESS_CREATED.value,
                )
            else:
                return self.response(
                    "Unable to create hauler.", status.HTTP_500_SERVER_ERROR.value
                )
        else:
            return self.response(
                "Error in creating resource.", status.HTTP_500_SERVER_ERROR.value
            )
